...
export default function CardRecadacaoDiaria() {
  ...
  async function buscarDados() {
    try {
      ...
      // divide pelos dias do mês → arrecadação diária
      const diasNoMes = new Date(anoAtual, mesAtual + 1, 0).getDate()
      const arrecadacaoDiaria = somaMensal / diasNoMes

      setTotal(arrecadacaoDiaria.toFixed(2))
    } catch (error) {
  ...
  return (
    <CardCustomPage
      ...
      description="Calculado a partir das saídas mensais"
      content={`R$: ${total}`}
