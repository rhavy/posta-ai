...
export default function ChardOrverviwWidegMesFinancias({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function buscarDados() {
      try {
        ...
        const sentidoValido = ["ENTRADA", "SAIDA"].includes(f.sentido);
        if (f.userId !== id || !f.criadoEm || !sentidoValido) return false;
