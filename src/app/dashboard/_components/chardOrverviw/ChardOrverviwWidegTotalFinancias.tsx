...
export default function ChardOrverviwWidegTotalFinancias({ id }: { id: string }) {
  const [chartData, setChartData] = useState<ChartDataPoint[]>([])

  useEffect(() => {
    async function buscarDados() {
      try {
        const res = await fetch(`/api/finances?userId=${id}`)
        ...
        const lancamentos = dados.filter(f =>
          f.userId === id &&
          f.criadoEm &&
          ["ENTRADA", "SAIDA"].includes(f.sentido)
        );
